...
def preprocess_asr(data_dir, processed_dir, backbone_ckpt=None, device="cpu"):
    ...
    Applies ASR text normalization to UPPERCASE and keeps apostrophes (').
    """
    ...
    def _norm_text(t: str) -> str:
        # UPPERCASE + keep apostrophes and spaces; strip everything else
        return re.sub(r"[^A-Z' ]+", "", (t or "").upper()).strip()
    ...
    for root, _, files in os.walk(data_dir):
        for file in files:
            ...
            # >>> CHANGED: normalize to UPPERCASE, keep ' and spaces
            transcript = _norm_text(transcript_map.get(name_no_ext, ""))
            if not transcript:
        ...
        for wav in sorted(os.listdir(processed_dir)):
            ...
            for suf in ("_noise", "_pitch", "_stretch"):
                if lookup.endswith(suf):
                    ...
                    break
            # >>> CHANGED: normalize to UPPERCASE, keep ' and spaces
            transcript = _norm_text(transcript_map.get(lookup, ""))
...
def preprocess_emotion(data_dir, processed_dir, backbone_ckpt=None, device="cpu"):
    ...
    # HARMONIZED 6-CLASS INVENTORY (order fixed → ids 0..5)
    ...
    # Dataset-specific raw→canonical maps (pre-harmonization)
    ...
    # ── 4) Write labels.csv using harmonized 6-class EMO2ID ─────────────────────
    kept, dropped = 0, 0
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for fn in sorted(os.listdir(processed_dir)):
            ...
            # 1) IEMOCAP lookup (English only, limited to 4 base labels here)
            ...
            # ── HARMONIZE to 6 classes:
            # calm → neutral ; surprise → DROP ; everything else unchanged if in set
            if canon == "calm":
                canon = "neutral"
            if canon == "surprise":
                canon = None

            # Only keep files that map into the 6-class inventory
            if canon not in EMO2ID:
                dropped += 1
                continue
    ...
    print(f"✅ Done: labels.csv generated with {kept} kept entries and {dropped} dropped (out-of-inventory) in {processed_dir!r}")
